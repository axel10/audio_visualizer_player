...
static float* g_fft_in = nullptr;
static float* g_fft_out = nullptr;
static float* g_fft_work = nullptr;
...
int32_t fft_internal_create(int32_t fft_size) {
    ...
    if (g_fft_in) { pffft_aligned_free(g_fft_in); g_fft_in = nullptr; }
    if (g_fft_out) { pffft_aligned_free(g_fft_out); g_fft_out = nullptr; }
    if (g_fft_work) { pffft_aligned_free(g_fft_work); g_fft_work = nullptr; }
    ...
    g_fft_in = (float*)pffft_aligned_malloc((size_t)fft_size * sizeof(float));
    g_fft_out = (float*)pffft_aligned_malloc((size_t)fft_size * sizeof(float));
    g_fft_work = (float*)pffft_aligned_malloc((size_t)fft_size * sizeof(float));

    if (!g_fft_in || !g_fft_out || !g_fft_work) {
        if (g_fft_in) pffft_aligned_free(g_fft_in);
        if (g_fft_out) pffft_aligned_free(g_fft_out);
        if (g_fft_work) pffft_aligned_free(g_fft_work);
        g_fft_in = nullptr; g_fft_out = nullptr; g_fft_work = nullptr;
        pffft_destroy_setup(g_fft_setup);
        g_fft_setup = nullptr;
        g_fft_size = 0;
        return -3;
...
int32_t fft_internal_compute_spectrum(const float* input_samples, int32_t sample_count, float* out_magnitudes, int32_t out_count) {
    std::lock_guard<std::mutex> lock(g_fft_mutex);
    if (g_fft_setup == nullptr || g_fft_size <= 0 || !g_fft_in || !g_fft_out || !g_fft_work) return -1;
    if (input_samples == nullptr || out_magnitudes == nullptr) return -1;

    // Apply Hanning Window
    for (int32_t i = 0; i < g_fft_size && i < sample_count; ++i) {
        float window = 0.5f - (0.5f * cosf((2.0f * 3.14159265f * (float)i) / (float)(g_fft_size - 1)));
        g_fft_in[i] = input_samples[i] * window;
    }
    for (int32_t i = sample_count; i < g_fft_size; ++i) {
        g_fft_in[i] = 0.0f;
    }

    pffft_transform_ordered(g_fft_setup, g_fft_in, g_fft_out, g_fft_work, PFFFT_FORWARD);
...
int32_t fft_internal_compute_compressed_bands(const float* magnitudes, int32_t bins, float* out_bands, int32_t band_count) {
    ...
    if (magnitudes == nullptr) return -20;
